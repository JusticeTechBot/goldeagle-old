    ...
    for token in tokens:
        ...
        for i in range(1):
            ...
            if "error" in response:
                print(f"{RED}Error on request {i + 1}/1: {response['error']}{RESET}")
            else:
                print(f"{GREEN}Success on request {i + 1}/: {response}{RESET}")

            delay = random.uniform(2, 5)
            print(f"{YELLOW}Calculated delay: {delay:.2f} seconds{RESET}")
            time.sleep(delay)
        
        print(f"{BLUE}Completed requests for token {token}.{RESET}")
